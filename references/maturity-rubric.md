...
| 5 | 4 + 认知债代理指标（无测试 AI commit 比例、文件作者集中度）+ 有足够长的时间序列，能看出各指标的**方向**而不只是当期值 |

证据：项目里有没有度量脚本 / 看板；本 skill 的基线文件是否连续；`--trend` 的时间序列是否够长（一个季度窗口只是一个点，看不出方向）。
**不要**因为团队「能说出自己在 J 曲线的哪一段」就给 5 分——J 曲线是描述性比喻，没有公认测量方法，说得出位置不等于测得出。
