...
| 7 | 度量与经济账 | DORA 四指标（J 曲线只作提醒，不作评分口径）| 有没有 DORA 指标、AI 归属率、返工率、认知债代理 |
...
- **J 曲线是比喻，不是标准**。DORA 与生产率经济学文献用它描述「先降后升」的采用形态，但它**没有公认的测量方法、没有阈值、没有基准数据集**。可以用它提醒自己「窗口要拉长」，不能拿它当评分或对外汇报的结论（「我们已在上升沿」不是一个可核查的陈述）。要对标就用 DORA 四指标。
- **返工率没有及格线**。`fix/(fix+feat)` 是构成比，强烈受生命周期阶段影响：维护期本就以修为主，开荒期本就以功能为主，发版前稳定期冲高是正常的。只能同团队纵比，且要先确认「可分类提交比例」本身没变——否则趋势是提交规范变化造成的假象。
